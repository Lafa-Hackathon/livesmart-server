...
if ($basename == 'videoai.php') { ?>
    <script>
        ...
        .then(
            function (completion) {
                const homeImages = document.getElementById('avatar_container');
                completion.response.avatars.forEach((avatar) => {
                    avatar.avatar_states.forEach((avatarUi) => {
                        if (avatarUi.id !== 'josh_lite3_20230714' && avatarUi.id !== 'josh_lite_20230714' && avatarUi.id !== 'Lily_public_lite1_20230601' &&
                            ...
                            avatarUi.id !== 'Mido-lite-20221128') {
        ...
        jQuery(document).ready(function ($) {
            ...
            $('#saveAvatars').click(function (event) {
                $('#error').hide();
                $('#success').hide();
                const file = $('#video-element-back-img')[0].files[0];
                var fileType = '';
                ...
                let roomid = ($('#room').val()) ? $('#room').val() : Math.random().toString(36).slice(2).substring(0, 10);
                let quality = $('#quality').val();
                let system = $('#system').val();
                let tools = ($('#tools_name').val() && $('#tools_description').val() && $('#tools_parameters').val()) ? $('#tools_name').val() + '|' + $('#tools_description').val() + '|' + $('#tools_parameters').val() : '';
                ...
                var dataObj = {'type': 'setvideoai', 'agenturl': agentUrl, 'roomId': roomid, 'video_ai_avatar' : videoAvatar, 'video_ai_name': videoAvatarName, 'video_ai_background': back_video, 'video_ai_quality': quality, 'video_ai_voice': videoAvatarVoice, 'video-element-back-img': video_file, 'video_ai_system': system, 'video_ai_tools': tools};
                var formData = new FormData();
                for (var key in dataObj) {
                    formData.append(key, dataObj[key]);
                }
