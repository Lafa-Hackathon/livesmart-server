 ...
 * @return boolean|type
 */
function setVideoAi($agenturl, $roomId, $video_ai_avatar, $video_ai_name, $video_ai_background, $video_ai_quality, $video_ai_voice, $video_ai_system, $video_ai_tools, $video_back_image) {
    global $dbPrefix, $pdo;
    checkHeaders();
    try {
        $array = [$roomId];
        ...
        try {
            $sql = "INSERT INTO " . $dbPrefix . "rooms (agenturl, roomId, agent_id, video_ai_avatar, video_ai_name, video_ai_background, video_ai_voice, video_ai_quality, video_ai_system, video_ai_tools) "
                    . "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $pdo->prepare($sql)->execute([$agenturl, $roomId, $_SESSION["agent"]['tenant'], $video_ai_avatar, $video_ai_name, $image_name, $video_ai_voice, $video_ai_quality, $video_ai_system, $video_ai_tools]);
            return 200;
        } catch (Exception $e) {
            return 'Error';
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data) && $data->type) {
        ...
        if (isset($_POST['type']) && $_POST['type'] == 'setvideoai') {
            $video_back_image = (isset($_FILES['video-element-back-img'])) ? $_FILES['video-element-back-img'] : '';
            echo setVideoAi($_POST['agenturl'], $_POST['roomId'], $_POST['video_ai_avatar'], $_POST['video_ai_name'], $_POST['video_ai_background'], $_POST['video_ai_quality'], $_POST['video_ai_voice'], @$_POST['video_ai_system'], @$_POST['video_ai_tools'], $video_back_image);
        }
    }
}
